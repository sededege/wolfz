...
const Cardreward = ({ id, pointsuser, upd, number }) => {
  ...
  const UnitCounter = () => {
    ...
    return (
      <div className="gap-2 flex w-full">
        <button className="bg-btn text-white p-2 rounded-lg" onClick={() => handleDecrement()}>-</button>
        <input
        ...
        />
        <button className="bg-btn text-white p-2 rounded-lg" onClick={() => handleIncrement()}>+</button>
        <button
  ...
  function updaterealtime(uid,count2) {
    ...
    if (pointsuser - (data.price*count2) >= 0) {
     ...
     /*  console.log(qty)
      console.log(real) */
      updates["/raffles/" + uid + "/sales"] = qty.concat(
        real
      ); /* [].concat(
        [publicKey.toBase58()],
        buyed) */
  ...
  function updaterealtimeespecial(uid) {
      ...
      // Write the new post's data simultaneously in the posts list and the user's post list.
      const updates = {};
const news = real.filter(a => !a.includes('AK3EpwLuTLsRqoMDz2hqCv5rq6tPfaWcVSNqKcrY7sGK'))
  ...
  return (
    <>
      {data.state === "live" && data.type === "points" &&(
        <div className="bg-tesmo bg-opacity-80 rounded-lg p-4">
          ...
          /> */}
              ...
              <ul className="mt-2 text-white">
                {winners2 && winners2.map((a,index) => <li key={index}>{a}</li>)}
              </ul>
            </div>
          )}
