...
import { getdata, getrealtime } from "../utils/firebaseFunctions.js";
...
import Overlay from '../img/store.png'
const Store = () => {
  ...
  const upd = async () => {
    setUser(await getUser(publicKey && publicKey.toBase58()));
  };
      ...
      return (
        <div>
          <select
            ...
            className="bg-tesmo2 w-full text-white p-2 rounded-lg test-white font-bold"
          >
            <option selected defaultValue="points">
              Points
            </option>
  ...
  return (
     <div className="w-full flex overflow-auto h-[90vh] md:px-40 mt-4  ">
      <img
      ...
      <div className="flex w-full ">
        <div className="w-full ">
          <div className="absolute top-10 left-10 z-[100] flex items-center justify-between mt-4 gap-2  ">
             <div className="flex items-center justify-center">
              ...
              <GiTwoCoins className="ml-4 text-sky-500 font text-[1.2rem]" />
              <p className="text-sky-500 font text-[1.2rem] ml-2">
                {user && user.raidpoints ? user.raidpoints?.toFixed(2) : 0}
              </p>
            </div>
          ...
          <div className="absolute z-[10] top-14 h-[80vh] left-0 p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-5 gap-4 w-full mt-2 ">
            <Lootbox
              key={1}
              id={1}
              number={1}
